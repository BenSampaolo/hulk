...
class SimpleAMPBuilder:
    ...
    default_offset = [0.0, 0.0, 0.25, 0.25, -1.4, 1.4, 0.15, 0.15, -2.25, 2.25, -0.3, -0.3, 0.1, 0.1, 0.0, 0.0, 0.6, 0.6, -0.3, -0.3, -0.1, 0.1]

    def __init__(self, active_joints: list[str], action_scale: float = 1.0):
    ...
    def map_joints_back(self, joints: torch.Tensor) -> torch.Tensor:
        return joints[:, [0, 2, 4, 6, 8, 10, 1, 3, 5, 7, 9, 11]]
    ...
    def build_from_mocap_dict(self, raw_data: dict) -> np.ndarray | None:
        ...
        dof_pos = raw_data["dof_pos"] * self.action_scale + self.default_offset
        ...
        features = np.concatenate(
            [
                self.map_joints_back(dof_pos[:, indices]), 
                self.map_joints_back(dof_vel[:, indices]), 
                root_vel,
                root_ang_vel
...
class MjlabAMPOnPolicyRunner(VelocityOnPolicyRunner):
    ...
    def __init__(
        ...
        # action_scale is saved in the actions config
        action_scale = 0.5
        builder = SimpleAMPBuilder(self.active_joints, action_scale=action_scale)
    ...
    def _compute_amp_features(self, env: RslRlVecEnvWrapper, obs: torch.Tensor) -> torch.Tensor:
        ...
        root_vel = robot.data.root_link_lin_vel_b # [num_envs, 3]
        root_ang_vel = robot.data.root_link_ang_vel_b # [num_envs, 3]

        return torch.cat([j_pos, j_vel, root_vel, root_ang_vel], dim=-1)
