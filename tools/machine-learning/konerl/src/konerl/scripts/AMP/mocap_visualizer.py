...
from konerl.scripts.AMP import MocapBuffer
...
class ViserAMPViewer(ViserPlayViewer):
    def __init__(
        self,
        env: EnvProtocol,
        ...
        super().__init__(env, policy, frame_rate, verbosity, viser_server=viser_server, checkpoint_manager=checkpoint_manager)
        self.mocap_file_path = "motions/CMU/11/"

        self.active_joints = [
            #'ALeft_Shoulder_Pitch', 'ARight_Shoulder_Pitch', 'Left_Shoulder_Roll', 'Right_Shoulder_Roll', 'Left_Elbow_Pitch', 'Right_Elbow_Pitch', 'Left_Elbow_Yaw', 'Right_Elbow_Yaw', 
        ...
        self.mocap_builder = SimpleAMPBuilder(active_joints=self.active_joints)

        self.mocap_buffer = MocapBuffer.load(data_dir=Path("./motions/CMU/11/"), history_length=1, device="cpu", builder=self.mocap_builder)
    ...
    def _execute_step(self) -> bool:
        ...
        try:
            with torch.no_grad():
                actions = self.mocap_buffer.next().squeeze()[:len(self.active_joints)]
                # actions -= getattr(self.env.cfg.scene.entities["robot"], "init_state", torch.zeros_like(actions))
                actions -= torch.tensor([0.0, 0.0, 0.25, -1.4, 0.15, -2.25, 0.25, 1.4, 0.15, 2.25, -0.3, 0.1, 0.0, 0.6, -0.3, -0.1, -0.3, -0.1, 0.0, 0.6, -0.3, 0.1])[22-len(self.active_joints):]
                actions *= getattr(self.env.cfg.actions["joint_pos"], "scale", 1.0)
                actions = torch.cat([actions, torch.zeros(1)], dim=0).unsqueeze(0)
...
def run_play(task_id: str, cfg: PlayConfig):
  ...
  if resolved_viewer == "native":
    ...
    ViserAMPViewer(env, policy, checkpoint_manager=ckpt_manager).run()
  else:
