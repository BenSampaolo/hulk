...
class MocapBuffer:
    ...
    def load(cls, data_dir: Path, builder, history_length: int, device: torch.device | str):
        ...
        for f in files:
            try:
                ...
                print(f"[AMP] Loaded {f.name} with {features.shape[0]} frames and {features.shape[1]} features.")
    ...
    def next(self) -> torch.Tensor:
        """Returns the next sequential window of observations, for visualization."""
        if not hasattr(self, "_current_index"):
            self._current_index = 0
        
        if self._current_index >= self.valid_indices.size(0):
            self._current_index = 0
        
        obs_window = self.observations[self._current_index, :]
        
        self._current_index += 1
        print(f"[AMP] Visualizer next() index: {self._current_index}/{self.valid_indices.size(0)}")
        return obs_window.unsqueeze(0)