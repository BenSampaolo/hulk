...
class SmoothedVelocityCommand(UniformVelocityCommand):
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        ...
        # 2. Zero-Division Fix: Clamp denominator
        u_norm = u / torch.clamp_min(torch.norm(u, dim=1, keepdim=True), 1e-6)

        # 3. Outer-Edge Bias: Use power > 3.0 (e.g., 5.0) to reduce inner volume density
        ...
        if len(init_vel_env_ids) > 0:
            ...
            # Prevent lag on initialization by snapping EMA to the target
    ...
    def _update_command(self) -> None:
        ...
        # Apply EMA to the entire command tensor
        self.vel_command_b = (
        ...
        # Ensure standing environments remain zeroed
