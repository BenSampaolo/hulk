...
impl SensorFrame {
    pub fn validate(&self, now: Time, parameters: &Parameters) -> Result<()> {
        validate_time(
            now,
            self.timestamp,
            "sensor frame",
            parameters.timing.maximum_input_age,
        )?;
        ensure!(
            self.position
                .into_iter()
...
pub(crate) fn validate_time(
    ...
    maximum_age: Duration,
        ...
        now >= timestamp && now.duration_since(timestamp) <= maximum_age,
        "stale or future {source}"
    );
    Ok(())
}
