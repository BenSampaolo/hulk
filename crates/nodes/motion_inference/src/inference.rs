...
impl Inference {
    ...
    fn validate_update(
        ...
        sensor.validate(now, &self.parameters)?;
        ...
        if let Some(last) = self.previous_update {
                ...
                now.duration_since(last) <= self.parameters.timing.maximum_input_age,
                "controller update gap exceeded maximum input age"
            );
        }
        Ok(())
    }
